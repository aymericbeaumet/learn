...
Ok, so we've just seen how to create a variable. But it's empty, or
undefined as we call it more formally. That's great but it's not very
exciting. Let's see how we can push it one step further by storing text into
our variables.
...
You can use the + operator to join strings together (this operation is
called concatenation).

To assign a string to a variable, use the = sign. You can change a value of
...
3. Concatenate `firstname` and `lastname` separated by a space into a
